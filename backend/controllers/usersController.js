...
exports.registerUser = async (req, res) => {
  const { firstName, lastName, email, academicYear, major, role, password } =
    req.body;
  ...
  try {
    ...
    await user.save();
    console.log("(register) User added to the database");
    ...
    res
      .status(201)
      .json({ token, userId: user._id, firstName, lastName, role });
  } catch (error) {
...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    console.log("(login) Token generation succesfull");
...
// Use this to check if a user is logged in or not
